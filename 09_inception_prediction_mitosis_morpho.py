...
def main(args):
    ...
    mitosis_morpho_model_json = f'{model_dir}{model_name}/morphodynamic_mitosis_densenet.json'
    ...
    for index, t_initial in enumerate(t_initials):
        ...
        for trackmate_id in tqdm(tracks_dataframe_short['TrackMate Track ID'].unique()):
            gbr_prediction[trackmate_id] = inception_model_prediction(tracks_dataframe_short, trackmate_id, tracklet_length, class_map_mitosis, morphodynamic_model=mitosis_morpho_torch_model, device=device)

        filtered_gbr_prediction = {k: v for k, v in gbr_prediction.items() if v is not None and v != "UnClassified"}
        save_cell_type_predictions(tracks_dataframe_short, class_map_mitosis, filtered_gbr_prediction, mitosis_prediction_dir, channel)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_name', type=str, default='morphodynamic_feature_mitosis_25_growth_rate_16', help='Model name including full path')
