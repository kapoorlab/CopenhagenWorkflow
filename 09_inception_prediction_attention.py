...
gbr_shape_lightning_model, gbr_shape_torch_model = LightningModel.extract_mitosis_model(
    ...
    local_model_path=os.path.join(home_folder, f'Mari_Models/TrackModels/shape_feature_lightning_attention_gbr_{tracklet_length}_{channel}shallowest_litest/')
...
gbr_dynamic_lightning_model, gbr_dynamic_torch_model = LightningModel.extract_mitosis_model(
    ...
    Adam,
    map_location=torch.device(device),
    local_model_path=os.path.join(home_folder, f'Mari_Models/TrackModels/dynamic_feature_lightning_attention_gbr_{tracklet_length}_{channel}shallowest_litest/')
)
...
for index, t_initial in enumerate(t_initials):
        ...
        annotations_prediction_dir = f'{home_folder}Mari_Data_Oneat/Mari_{dataset_name}_Dataset_Analysis/annotations_predicted_attention_tracklet_length_{tracklet_length}_t_initial_{t_initial}_t_final_{t_final}_{channel}shallowest_litest/'
